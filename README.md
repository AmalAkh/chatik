...
> Za každý deň oneskoreného odovzdania je tímu odobratých 25% bodov z pôvodného maxima (deň po termíne tím získa 3/4 bodov, dva dni po termíne 1/2, atď.)insert symbol at the start of each line in vs code
...
> 
> Dokumentácia musí obsahovať minsert symbol at the start of each line in vs codeinimálne tieto časti:
> 
> - zadanie
...
## UML Class diagram 
<img src="diagrams/classes.drawio.png">

## Entity-Relationship diagaram
#App Database diagram
Generated using [DbSchema](https://dbschema.com)

![img](./diagrams/MainDiagram.svg)
...
|Type |Name |On |
|---|---|---|
| &#128273;  | api\_tokens\_pkey | ON id|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | fk_api_tokens_users | ( user\_id ) ref [public.users](#users) (id) |




### Table public.channel_bans 
|Idx |Name |Data Type |
|---|---|---|
| * &#128273;  | id| serial  |
| &#11016; | channel\_id| integer  |
...
|Type |Name |On |
|---|---|---|
| &#128273;  | channel\_bans\_pkey | ON id|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | channel_bans_channel_id_foreign | ( channel\_id ) ref [public.channels](#channels) (id) |
|  | channel_bans_user_id_foreign | ( user\_id ) ref [public.users](#users) (id) |
...
|Type |Name |On |
|---|---|---|
| &#128273;  | channel\_members\_pkey | ON id|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | channel_members_channel_id_foreign | ( channel\_id ) ref [public.channels](#channels) (id) |
|  | channel_members_user_id_foreign | ( user\_id ) ref [public.users](#users) (id) |
...
| &#11016; | owner\_id| integer  |
| * | created\_at| timestamptz  DEFAULT CURRENT_TIMESTAMP |
| * | updated\_at| timestamptz  DEFAULT CURRENT_TIMESTAMP |
...
| &#128269;  | channels\_name\_unique | ON name|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | channels_owner_id_foreign | ( owner\_id ) ref [public.users](#users) (id) |
...
| &#128270;  | channel\_messages\_channel\_id\_created\_at\_index | ON channel\_id, date|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | messages_user_id_foreign | ( user\_id ) ref [public.users](#users) (id) |
|  | messages_channel_id_foreign | ( channel\_id ) ref [public.channels](#channels) (id) |
...
|Type |Name |On |
|---|---|---|
| &#128273;  | push\_subscriptions\_pkey | ON id|
| &#128269;  | push\_subscriptions\_end\_point\_unique | ON end\_point|

##### Foreign Keys
|Type |Name |On |
|---|---|---|
|  | push_subscriptions_user_id_foreign | ( user\_id ) ref [public.users](#users) (id) |
...
|---|---|---|
| &#128273;  | users\_pkey | ON id|
| &#128269;  | users\_nickname\_unique | ON nickname|
| &#128269;  | users\_email\_unique | ON email|

##### Constraints
|Name |Definition |
|---|---|
| users_status_check | status = ANY (ARRAY['online'::text, 'dnd'::text, 'offline'::text]) |


# Libraries

## Addtional libraries for front-end:
- axios for easy HTTP requests
- socket.io-client for real-time messaging using socket.io
- vue-router for spa routing
- register-service-worker used o simplify service worker registration, used creating PWA
## Additional libraries for backend-end:
- adonisjs-scheduler - scheduler for AdonisJS V5,  used for deleting of chats old chats
- socket.io used for real-time communication using sockets
- web-push - utility library for web push notifications
